...
st.set_page_config(
    page_title="Creador de Videos Tech Pro", page_icon="🎬", layout="wide"
)
  ...
  try:
    ...
    for page in pages.values():
      ...
      if info:
        ...
        if img_url and img_url.lower().endswith(
            (".jpg", ".jpeg", ".png", ".webp")
        ):
...
def crear_frame_diapositiva(
  ...
  if logo_img:
    try:
      ...
      logo.thumbnail(
          (int(140 * f_factor), int(140 * f_factor)), LANCZOS_FILTER
      )
  ...
  if H > W:  # Vertical (9:16)
    ...
    draw.rounded_rectangle(
        [x_img - 6, y_img - 6, x_img + img.width + 6, y_img + img.height + 6],
        radius=12,
        fill=colores["card"],
    )
    ...
    bottom_specs = min(
        top_specs + len(lineas) * line_height + 60, int(H * 0.94)
    )
    draw.rounded_rectangle(
        [int(W * 0.05), top_specs, int(W * 0.95), bottom_specs],
        radius=20,
        fill=colores["card"],
        outline=colores["accent"],
        width=3,
  ...
  else:  # Horizontal (16:9) o Cuadrado (1:1)
    img = imagen_base.convert("RGBA")
    ...
    draw.rounded_rectangle(
        [int(W * 0.50), int(H * 0.12), int(W * 0.95), int(H * 0.22)],
        radius=12,
        fill=colores["card"],
        outline=colores["accent"],
        width=3,
    ...
    lineas = formatear_lineas_specs(
        texto_specs, draw, font_specs, max_width=int(W * 0.40)
    )
    line_height = int(48 * f_factor)
    bottom_specs = min(
        top_specs + len(lineas) * line_height + 50, int(H * 0.90)
    )
    draw.rounded_rectangle(
        [int(W * 0.50), top_specs, int(W * 0.95), bottom_specs],
        radius=18,
        fill=colores["card"],
        outline=colores["accent"],
        width=3,
    ...
    for lin in lineas:
      draw.text(
          (int(W * 0.53), y_txt), lin, font=font_specs, fill=colores["text"]
      )
      y_txt += line_height
...
st.caption(
    "Personalización completa: temas, formatos, música de fondo, marcas de"
    " agua y gráficos."
)
...
with col_left:
  ...
  handle_social = st.text_input(
      "🏷️ Tu usuario / Canal (Marca de agua):", value="@TuCanalTech"
  )
...
with col_right:
  ...
  velocidad_voz = st.select_slider(
      "Velocidad de locución:",
      options=["-20%", "-10%", "+0%", "+10%", "+25%"],
      value="+0%",
  )

  archivo_musica = st.file_uploader(
      "🎵 Música de Fondo (Opcional - MP3):", type=["mp3", "wav"]
  )
  # Rango ajustado para música suave (1% al 20%, por defecto 4%)
...
with col_logo:
  uploaded_logo = st.file_uploader(
      "🖼️ Logo de tu Canal (PNG transparente):", type=["png"]
  )
  logo_image = Image.open(uploaded_logo) if uploaded_logo else None

with col_phone:
  nombre_celular = st.text_input(
      "📱 Modelo del Celular / Producto:", value="Poco X6 Pro"
  )
  num_escenas = st.number_input(
      "🔢 Número de Escenas:", min_value=1, max_value=6, value=3, step=1
  )
  ...
  with st.expander(f"🎬 Escena {i+1}", expanded=(i == 0)):
    ...
    with c1:
      ...
      loc = st.text_area(
          f"Locución hablada:",
          value=(
              f"El {nombre_celular} ofrece máxima potencia e increíble"
              " pantalla."
          ),
          key=f"l_{i}",
      )
      pts_def = (
          ...
          if i == 0
              ...
              "Cámara: 64 MP OIS + 8 MP\nBatería: 5000 mAh (67W)\n⭐ Rendimiento:"
              " 9.0/10"
          )
      )
      pts = st.text_area(
          f"Specs / Gráficos en pantalla:", value=pts_def, key=f"p_{i}"
      )
    ...
    with c2:
      ...
      orig_img = st.radio(
          "Origen:",
          ["Buscar en Web", "Subir Foto", "URL Directa"],
          key=f"oi_{i}",
      )
      ...
      if orig_img == "Subir Foto":
        f_up = st.file_uploader(
            f"Foto escena {i+1}:", type=["jpg", "png", "webp"], key=f"fu_{i}"
        )
        if f_up:
      ...
      kw_search = (
          st.text_input(
              "Búsqueda web:", value=f"{nombre_celular} phone", key=f"kw_{i}"
          )
          if orig_img == "Buscar en Web"
          else ""
      )
      ...
      if st.button(f"👁️ Previsualizar Escena {i+1}", key=f"prev_{i}"):
        img_t = (
            img_escena
            if (orig_img != "Buscar en Web" and img_escena)
            else buscar_imagen_real_web(kw_search)
        )
        dims = FORMATOS[formato_elegido]
        f_prev = crear_frame_diapositiva(
            img_t,
            tit,
            pts,
            tema_elegido,
            dims,
            handle_usuario=handle_social,
            logo_img=logo_image,
        )
        st.image(
            f_prev,
            caption=f"Vista previa ({formato_elegido.split()[0]})",
            width=360,
        )
    ...
    escenas_config.append({
        ...
        "duracion_modo": dur_m,
        "duracion_fija": dur_f,
        "origen_imagen": orig_img,
        "imagen_escena": img_escena,
        "busqueda_tag": kw_search,
    })
...
if st.button("🚀 Renderizar Video Profesional", type="primary") and nombre_celular:
  try:
    ...
    for i, escena in enumerate(escenas_config):
      st.info(
          f"🎬 Procesando Escena {i+1}/{len(escenas_config)}: {escena['titulo']}"
      )

      # 1. Generar Audio TTS de Voz
      with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as t_audio:
        generar_audio(
            escena["texto_locucion"],
            t_audio.name,
            voz=voz_code,
        ...
        # Refuerzo del 30% en el volumen de la voz para sobreponerse a la música
        audio_voz_clip = ajustar_volumen(audio_voz_clip, 1.3)
        dur_audio = audio_voz_clip.duration

      # 2. Calcular Duración
      duracion_final = (
          max(escena["duracion_fija"], dur_audio)
          if escena["duracion_modo"] == "Tiempo Fijo (Seg)"
          else dur_audio
      )

      # 3. Obtener Imagen
      if (
          escena["origen_imagen"] in ["Subir Foto", "URL Directa"]
          and escena["imagen_escena"] is not None
      ):
        img_base = escena["imagen_escena"]
      else:
        img_base = buscar_imagen_real_web(
            escena["busqueda_tag"]
            if escena["busqueda_tag"]
            else f"{nombre_celular} phone"
        )
      ...
      if clip_musica_global:
        ...
        # Se aplica el volumen fino seleccionado en el slider
