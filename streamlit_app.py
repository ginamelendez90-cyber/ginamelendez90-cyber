...
st.set_page_config(page_title="MLB Analyst - Proyección de Hit", page_icon="⚾", layout="wide")

st.title("⚾ Analizador MLB: Proyección y Justificación de Hit")
st.markdown("Análisis sabermétrico automatizado con explicación cualitativa y gráficos interactivos.")
st.markdown("---")
...
if jugador_sel:
    ...
    with col_g1:
        ...
        if df_15 is not None and not df_15.empty:
            ...
            fig_trend = px.line(
                ...
                y='AVG_Movil', 
                markers=True,
                labels={'AVG_Movil': 'Promedio Móvil', 'Fecha': 'Fecha'},
    ...
    with st.expander("🧮 Ver Desglose de Fórmulas Matemáticas"):
        ...
        1. **Promedio Ponderado Base (AVG base):**  
            40% · Racha ({res['avg_reciente']:.3f}) + 60% · Temporada ({res['avg_season']:.3f}) = **{res['avg_base']:.3f}**
        
        2. **Ajustes por Entorno (AVG proyectado):**  
            Base ({res['avg_base']:.3f}) × Picheo ({res['factor_picheo']:.2f}) × Parque ({res['factor_parque']:.2f}) = **{res['avg_proyectado']:.3f}**
        
        3. **Distribución de Poisson:**  
            \(\lambda\) = {res['avg_proyectado']:.3f} × 3.8 AB = **{res['lambda_hits']:.2f} hits esperados**  
            Probabilidad de al menos 1 Hit = **{res['prob_hit']}%**
        """)
    ...
    if df_5_tabla is not None and not df_5_tabla.empty:
        ...
        cols_mostrar = [c for c in df_5_tabla.columns if c != 'AVG_Movil']
