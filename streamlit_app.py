...
# ==========================================
st.set_page_config(
    page_title="MLB Pro Match Odds & Live Tracker",
    layout="wide",
    page_icon="⚾",
)

# Inicializar historial de apuestas en session_state
...
# ==========================================
# 2. CONSTANTES Y PARÁMETROS SABERMÉTRICOS
# ==========================================
PARK_FACTORS = {
    "Coors Field": 1.15,
    "Fenway Park": 1.06,
...
# ==========================================
# 3. BARRA LATERAL (SIDEBAR) DE CONFIGURACIÓN
# ==========================================
...
ajuste_fatiga_bp = st.sidebar.checkbox(
    "Penalización por Fatiga de Bullpen (>1.30 WHIP)", value=True
)
...
# ==========================================
# 4. FUNCIONES AUXILIARES Y CÁLCULOS
# ==========================================
def parse_float(val, default=0.0) -> float:
    """Convierte de forma segura un valor a flotante."""
...
def generar_campo_svg(offense_dict: dict, hit_x=None, hit_y=None) -> str:
...
def evaluar_apuesta_partido(apuesta: dict, live_data: dict, status_juego: str):
    ...
    f5_away = sum(
        [parse_float(inn.get("away", {}).get("runs", 0)) for inn in innings[:5]]
    )
    f5_home = sum(
        [parse_float(inn.get("home", {}).get("runs", 0)) for inn in innings[:5]]
    )
    f5_total = f5_away + f5_home
    f5_completado = len(innings) >= 5 and (
        linescore.get("currentInning", 0) > 5 or "Final" in str(status_juego)
    )
    ...
    if tipo == "Victoria (Moneyline)":
        ...
        if mi_score > rival_score:
            return (
                "✅ GANADA" if es_final else "🟢 GANANDO",
                f"{int(mi_score)} - {int(rival_score)}",
            )
        elif mi_score < rival_score:
            return (
                "❌ PERDIDA" if es_final else "🔴 PERDIENDO",
                f"{int(mi_score)} - {int(rival_score)}",
            )
        else:
            return (
    ...
    # 2. RUN LINE / HÁNDICAP DEL PARTIDO
        ...
        if score_con_handicap > rival_score:
            return (
                "✅ GANADA" if es_final else "🟢 CUBRIENDO",
                f"Con Hándicap: {score_con_handicap:.1f} vs {int(rival_score)}",
            )
        elif score_con_handicap < rival_score:
            return (
                "❌ PERDIDA" if es_final else "🔴 SIN CUBRIR",
                f"Con Hándicap: {score_con_handicap:.1f} vs {int(rival_score)}",
            )
        else:
            return (
                "🟡 EMPATE",
                f"{score_con_handicap:.1f} vs {int(rival_score)}",
            )
    ...
    # 3. TOTAL DE CARRERAS DEL PARTIDO (OVER / UNDER)
        ...
        if tipo == "Total: OVER (Altas)":
            if total_carreras > linea:
                return (
                    "✅ GANADA" if es_final else "🟢 CUBIERTA",
                    f"{int(total_carreras)} carreras (Línea: {linea})",
                )
            else:
                faltan = linea - total_carreras
                return (
                    "❌ PERDIDA" if es_final else "🔴 EN PROGRESO",
                    f"Llevan {int(total_carreras)} | Faltan {faltan:.1f}",
                )
        else:
            if total_carreras < linea:
                return (
                ...
                return (
                    "❌ PERDIDA" if es_final else "🔴 SUPERADA",
                    f"Llevan {int(total_carreras)} | Máximo: {linea}",
                )
    ...
    # 4. MERCADOS DE PRIMERAS 5 ENTRADAS (F5)
        ...
        if mi_f5 > rival_f5:
            return (
                "✅ GANADA" if f5_completado else "🟢 GANANDO F5",
                f"F5: {int(mi_f5)} - {int(rival_f5)}",
            )
        elif mi_f5 < rival_f5:
            return (
                "❌ PERDIDA" if f5_completado else "🔴 PERDIENDO F5",
                f"F5: {int(mi_f5)} - {int(rival_f5)}",
            )
        else:
        ...
        if tipo == "F5 Total: OVER (Altas)":
            if f5_total > linea:
                return (
                    "✅ GANADA" if f5_completado else "🟢 CUBIERTA F5",
                    f"F5: {int(f5_total)} carreras (Línea: {linea})",
                )
            else:
                return (
                    "❌ PERDIDA" if f5_completado else "🔴 EN PROGRESO F5",
                    f"F5 Llevan {int(f5_total)} | Línea: {linea}",
                )
        else:
            if f5_total < linea:
                return (
                ...
                return (
                    "❌ PERDIDA" if f5_completado else "🔴 SUPERADA F5",
                    f"F5 Llevan {int(f5_total)} | Línea: {linea}",
                )
...
def calcular_fip(stats: dict) -> float:
    """Calcula la métrica FIP (Fielding Independent Pitching) de un abridor."""
    if not stats:
        return 4.20
...
def simular_monte_carlo(exp_away: float, exp_home: float, n_sims: int = 10000):
    """Ejecuta una simulación Monte Carlo con distribución de Poisson para proyectar probabilidades."""
    np.random.seed(42)
    ...
    return (
        prob_away,
        prob_home,
        np.mean(carreras_away),
        np.mean(carreras_home),
        np.mean(carreras_away + carreras_home),
    )


def calcular_ev(prob_modelo_pct: float, cuota_decimal: float) -> float:
    """Calcula el Valor Esperado (+EV) de una apuesta en porcentaje."""
    prob_decimal = prob_modelo_pct / 100.0
    return round(((prob_decimal * cuota_decimal) - 1) * 100, 2)


# ==========================================
# 5. CONSULTAS A LA API CON CACHÉ
# ==========================================
...
def obtener_whip_bullpen(team_id):
    try:
        team_stats = statsapi.get(
            "team_stats",
            {"teamId": team_id, "statType": "season", "group": "pitching"},
        )
        for stat in team_stats.get("stats", []):
            if stat.get("type", {}).get("displayName") == "season":
                ...
                if splits:
                    return parse_float(
                        splits[0].get("stat", {}).get("whip", 1.30), 1.30
                    )
        return 1.30
    except Exception:
        return 1.30
...
# ==========================================
# 6. EJECUCIÓN PRINCIPAL DE LA APLICACIÓN
# ==========================================
...
if not juegos:
    ...
    lista_juegos = [
        f"{j['away_name']} @ {j['home_name']} - Estado: {j['status']}"
        for j in juegos
    ]
    ...
    with st.sidebar.form("form_apuesta_partido"):
        ...
        linea_valor = st.number_input(
            "Línea / Hándicap (Ej. -1.5, +1.5, 8.5):", value=8.5, step=0.5
        )
        monto_apostado = st.number_input("Monto ($):", value=10.0, step=5.0)
        cuota_apostada = st.number_input(
            "Cuota / Odds (Decimal):", value=1.90, step=0.05
        )
        ...
        if btn_guardar:
            ...
            nueva_apuesta = {
                ...
                "seleccion": (
                    seleccion_equipo
                    if ("Victoria" in tipo_apuesta or "Run Line" in tipo_apuesta)
                    else desc_sel
                ),
                "linea": linea_valor,
    ...
    stats_p_away = (
        obtener_stats_jugador(away_pitcher.get("id"), "pitching")
        if away_pitcher.get("id")
        else {}
    )
    stats_p_home = (
        obtener_stats_jugador(home_pitcher.get("id"), "pitching")
        if home_pitcher.get("id")
        else {}
    )

    fip_away, fip_home = calcular_fip(stats_p_away), calcular_fip(stats_p_home)
    whip_bp_away, whip_bp_home = obtener_whip_bullpen(away_id), obtener_whip_bullpen(
        home_id
    )
    ...
    if ajuste_fatiga_bp:
        if whip_bp_home > 1.30:
            exp_runs_away += 0.25
        if whip_bp_away > 1.30:
            exp_runs_home += 0.25
    ...
    with tab_montecarlo:
        ...
        c1.metric(
            f"Prob. Victoria {away_name}", f"{prob_away:.1f}%", f"Proj: {sim_away:.2f} R"
        )
        c2.metric(
            f"Prob. Victoria {home_name}", f"{prob_home:.1f}%", f"Proj: {sim_home:.2f} R"
        )
        c3.metric(
            "Total Proyectado del Partido",
            f"{total_esperado:.2f} Carreras",
            f"Park Factor: {park_factor}x",
        )
    ...
    with tab_ev:
        ...
        if not cuotas_raw:
            st.info(
                "💡 **Modo Demo**: Se muestran cuotas simuladas. Añade tu API Key para datos en vivo de Pinnacle, DraftKings, etc."
            )
            cuotas_lista = [
            ...
            for juego in cuotas_raw:
                if (
                    away_name.lower() in juego.get("away_team", "").lower()
                    or home_name.lower() in juego.get("home_team", "").lower()
                ):
                ...
                filas_ev.append(
                    {
                        ...
                        f"Diagnóstico {away_name}": (
                            f"🚀 +EV (+{ev_away:.1f}%)" if ev_away > 2.0 else "❌ Sin Valor"
                        ),
                        f"Cuota {home_name}": item["home_odds"],
                        f"EV {home_name}": f"{ev_home:+.2f}%",
                        f"Diagnóstico {home_name}": (
                            f"🚀 +EV (+{ev_home:.1f}%)" if ev_home > 2.0 else "❌ Sin Valor"
                        ),
                    }
                )
    ...
    with tab_vivo:
        ...
        if st.session_state.mis_apuestas:
            ...
            apuestas_del_partido = [
                a for a in st.session_state.mis_apuestas if a.get("game_id") == game_id
            ]
            ...
            if apuestas_del_partido:
                ...
                for idx, ap in enumerate(apuestas_del_partido):
                    estado, detalle = evaluar_apuesta_partido(
                        ap, live_data, juegos[idx_juego]["status"]
                    )
        ...
        if entradas_lista:
            ...
            st.dataframe(
                pd.DataFrame(tabla_innings), use_container_width=True, hide_index=True
            )
            ...
            c_tot1.metric(
                f"Total {away_name}",
                f"R: {away_totals.get('runs', 0)} | H: {away_totals.get('hits', 0)} | E: {away_totals.get('errors', 0)}",
            )
            c_tot2.metric(
                f"Total {home_name}",
                f"R: {home_totals.get('runs', 0)} | H: {home_totals.get('hits', 0)} | E: {home_totals.get('errors', 0)}",
            )
        else:
