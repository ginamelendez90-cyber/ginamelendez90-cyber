...
st.set_page_config(page_title="MLB Pro Match Odds & Live Tracker", layout="wide", page_icon="âš¾")

# Inicializar el historial de apuestas de partido en la sesiÃ³n de Streamlit
if 'mis_apuestas' not in st.session_state:
    st.session_state.mis_apuestas = []

st.title("ðŸš€ Tracker y Analizador de Apuestas de Partido MLB")
st.markdown("---")

# --- TABLA ESTÃTICA DE PARK FACTORS Y CONSTANTES ---
PARK_FACTORS = {
    "Coors Field": 1.15,
    "Fenway Park": 1.06,
...
st.sidebar.header("âš™ï¸ Panel de Control SabermÃ©trico")
fecha_seleccionada = st.sidebar.date_input("Fecha de AnÃ¡lisis:", datetime.today())
n_simulaciones = st.sidebar.slider("Simulaciones Monte Carlo:", min_value=1000, max_value=25000, value=10000, step=1000)
ajuste_fatiga_bp = st.sidebar.checkbox("PenalizaciÃ³n por Fatiga de Bullpen (>1.30 WHIP)", value=True)

st.sidebar.markdown("---")
st.sidebar.header("ðŸ”„ ActualizaciÃ³n en Vivo")
auto_refresh = st.sidebar.toggle("Auto-refresh cada 10s", value=False)

st.sidebar.markdown("---")
st.sidebar.header("ðŸ”‘ ConexiÃ³n a Casas de Apuestas")
odds_api_key = st.sidebar.text_input("The Odds API Key:", type="password")

# --- FUNCIONES AUXILIARES Y EVALUACIÃ“N EXCLUSIVA DE APUESTAS DE PARTIDO ---
...
def generar_campo_svg(offense_dict, hit_x=None, hit_y=None):
    """Genera un grÃ¡fico dinÃ¡mico SVG del diamante de bÃ©isbol con corredores y punto de caÃ­da del batazo"""
...
def evaluar_apuesta_partido(apuesta, live_data, status_juego):
    """EvalÃºa exclusivamente lÃ­neas de PARTIDO (Moneyline, Run Line, Over/Under, F5)"""
    ...
    if tipo == "Victoria (Moneyline)":
        ...
        if mi_score > rival_score:
            return ("âœ… GANADA" if es_final else "ðŸŸ¢ GANANDO", f"{int(mi_score)} - {int(rival_score)}")
        elif mi_score < rival_score:
            return ("âŒ PERDIDA" if es_final else "ðŸ”´ PERDIENDO", f"{int(mi_score)} - {int(rival_score)}")
        else:
            return ("ðŸŸ¡ EMPATE TEMPORAL", f"{int(mi_score)} - {int(rival_score)}")

    # 2. RUN LINE / HÃNDICAP DEL PARTIDO (Ej. -1.5 o +1.5)
    elif tipo == "Run Line / HÃ¡ndicap":
        es_away = apuesta['seleccion'] == apuesta['away_name']
        mi_score = carreras_away if es_away else carreras_home
        rival_score = carreras_home if es_away else carreras_away
        ...
        if score_con_handicap > rival_score:
            return ("âœ… GANADA" if es_final else "ðŸŸ¢ CUBRIENDO", f"Con HÃ¡ndicap: {score_con_handicap:.1f} vs {int(rival_score)}")
        elif score_con_handicap < rival_score:
            return ("âŒ PERDIDA" if es_final else "ðŸ”´ SIN CUBRIR", f"Con HÃ¡ndicap: {score_con_handicap:.1f} vs {int(rival_score)}")
        else:
            return ("ðŸŸ¡ EMPATE", f"{score_con_handicap:.1f} vs {int(rival_score)}")
    ...
    # 3. TOTAL DE CARRERAS DEL PARTIDO (OVER / UNDER)
        ...
        if tipo == "Total: OVER (Altas)":
            if total_carreras > linea:
                return ("âœ… GANADA" if es_final else "ðŸŸ¢ CUBIERTA", f"{int(total_carreras)} carreras (LÃ­nea: {linea})")
            else:
                faltan = linea - total_carreras
                return ("âŒ PERDIDA" if es_final else "ðŸ”´ EN PROGRESO", f"Llevan {int(total_carreras)} | Faltan {faltan:.1f}")
        else:
            if total_carreras < linea:
                return ("âœ… GANADA" if es_final else "ðŸŸ¢ CUBIERTA", f"{int(total_carreras)} carreras (LÃ­nea: {linea})")
            else:
                return ("âŒ PERDIDA" if es_final else "ðŸ”´ SUPERADA", f"Llevan {int(total_carreras)} | MÃ¡ximo: {linea}")
    ...
    # 4. MERCADOS DE PRIMERAS 5 ENTRADAS (F5)
        ...
        if mi_f5 > rival_f5:
            return ("âœ… GANADA" if f5_completado else "ðŸŸ¢ GANANDO F5", f"F5: {int(mi_f5)} - {int(rival_f5)}")
        elif mi_f5 < rival_f5:
            return ("âŒ PERDIDA" if f5_completado else "ðŸ”´ PERDIENDO F5", f"F5: {int(mi_f5)} - {int(rival_f5)}")
        else:
            return ("ðŸŸ¡ EMPATE F5", f"F5: {int(mi_f5)} - {int(rival_f5)}")

    elif tipo in ["F5 Total: OVER (Altas)", "F5 Total: UNDER (Bajas)"]:
        linea = apuesta['linea']
        if tipo == "F5 Total: OVER (Altas)":
            if f5_total > linea:
                return ("âœ… GANADA" if f5_completado else "ðŸŸ¢ CUBIERTA F5", f"F5: {int(f5_total)} carreras (LÃ­nea: {linea})")
            else:
                return ("âŒ PERDIDA" if f5_completado else "ðŸ”´ EN PROGRESO F5", f"F5 Llevan {int(f5_total)} | LÃ­nea: {linea}")
        else:
            if f5_total < linea:
                return ("âœ… GANADA" if f5_completado else "ðŸŸ¢ CUBIERTA F5", f"F5: {int(f5_total)} carreras (LÃ­nea: {linea})")
            else:
                return ("âŒ PERDIDA" if f5_completado else "ðŸ”´ SUPERADA F5", f"F5 Llevan {int(f5_total)} | LÃ­nea: {linea}")

    return ("âšª PENDIENTE", "Esperando datos")
...
if not juegos:
    st.warning("âš ï¸ No se encontraron partidos programados para la fecha seleccionada.")
else:
    lista_juegos = [f"{j['away_name']} @ {j['home_name']} - Estado: {j['status']}" for j in juegos]
    juego_elegido = st.selectbox("ðŸŽ¯ Selecciona el partido a monitorear:", lista_juegos)
    ...
    st.sidebar.markdown("---")
    st.sidebar.header("ðŸŽŸï¸ Registrar Apuesta de Partido")
    with st.sidebar.form("form_apuesta_partido"):
        ...
        linea_valor = st.number_input("LÃ­nea / HÃ¡ndicap (Ej. -1.5, +1.5, 8.5):", value=8.5, step=0.5)
        ...
        btn_guardar = st.form_submit_button("âž• AÃ±adir Apuesta de Partido")
        
        if btn_guardar:
            # ConstrucciÃ³n de la descripciÃ³n segÃºn el tipo de apuesta de partido
            ...
            st.sidebar.success("Â¡Apuesta de partido agregada!")
    ...
    # PESTAÃ‘AS PRINCIPALES DE ANÃLISIS
    tab_montecarlo, tab_ev, tab_vivo = st.tabs([
        "ðŸŽ² SIMULACIÃ“N MONTE CARLO (PARTIDO)", 
        "ðŸ’° DETECTOR +EV EN CASAS",
        "ðŸ“ˆ MARCADOR Y TRACKER EN VIVO"
    ])

    # --- TAB 1: MONTE CARLO ---
    with tab_montecarlo:
        st.header("ðŸŽ° ProyecciÃ³n General del Partido")
        ...
        st.markdown("---")
        st.subheader("ðŸ“Š MÃ©tricas Clave de Pitcheo Colectivo / Abridores")
    ...
    with tab_ev:
        st.header("ðŸ’° Detector de Valor (+EV) en Mercados de Partido")
        cuotas_raw = obtener_cuotas_mlb_api(odds_api_key) if odds_api_key else None
        
        if not cuotas_raw:
            st.info("ðŸ’¡ **Modo Demo**: Se muestran cuotas simuladas. AÃ±ade tu API Key para datos en vivo de Pinnacle, DraftKings, etc.")
            cuotas_lista = [
        ...
        if cuotas_lista:
            ...
            for item in cuotas_lista:
                ...
                filas_ev.append({
                    ...
                    f"DiagnÃ³stico {away_name}": f"ðŸš€ +EV (+{ev_away:.1f}%)" if ev_away > 2.0 else "âŒ Sin Valor",
                    f"Cuota {home_name}": item['home_odds'],
                    f"EV {home_name}": f"{ev_home:+.2f}%",
                    f"DiagnÃ³stico {home_name}": f"ðŸš€ +EV (+{ev_home:.1f}%)" if ev_home > 2.0 else "âŒ Sin Valor"
                })
            st.dataframe(pd.DataFrame(filas_ev), use_container_width=True, hide_index=True)

    # --- TAB 3: TRANSMISIÃ“N Y TRACKER EN VIVO DE APUESTAS ---
    with tab_vivo:
        st.header("ðŸŸï¸ Tracker en Vivo: Estado del Partido y Tickets")
        ...
        # --- SECCIÃ“N: TRACKER DE APUESTAS DE PARTIDO REGISTRADAS ---
        if st.session_state.mis_apuestas:
            st.subheader("ðŸŽ¯ Tus Apuestas de Partido en Seguimiento")
            ...
            if st.button("ðŸ—‘ï¸ Limpiar Historial de Apuestas"):
                st.session_state.mis_apuestas = []
                st.rerun()
        ...
        st.subheader("ðŸ“Œ Diamante y Duelo en Vivo")
        ...
        if entradas_lista:
            ...
            st.subheader("ðŸ“Š Marcador por Entradas (Linescore)")
