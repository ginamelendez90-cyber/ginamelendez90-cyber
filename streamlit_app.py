...
$$40\% \\cdot \\text{{Racha}} ({res['avg_reciente']:.3f}) + 60\% \\cdot \\text{{Temporada}} ({res['avg_season']:.3f}) = {res['avg_base']:.3f}$$Ajustes por Entorno (AVG proyectado):$$\\text{{Base}} ({res['avg_base']:.3f}) \\times \\text{{Picheo}} ({res['factor_picheo']:.2f}) \\times \\text{{Parque}} ({res['factor_parque']:.2f}) = {res['avg_proyectado']:.3f}$$Distribución de Poisson:$$\\lambda = \\text{{AVG}}_{{\\text{{proyectado}}}} \\times 3.8 \\text{{ AB}} = {res['lambda_hits']:.2f} \\text{{ hits esperados}}$$$$P(X \\ge 1) = 1 - e^{{-\\lambda}} = \\mathbf{{{res['prob_hit']}\\%}}$$    """)
