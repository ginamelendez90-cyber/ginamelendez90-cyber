...
fecha_seleccionada = st.sidebar.date_input(
    "Fecha de Análisis:", datetime.today()
)
...
if not juegos:
  ...
  away_name, home_name = (
      juegos[idx_juego]["away_name"],
      juegos[idx_juego]["home_name"],
  )

  # --- FORMULARIO SIDEBAR: APUESTAS DE PARTIDO ---
  st.sidebar.markdown("---")
  st.sidebar.header("🎫 Registrar Apuesta de Partido")
  with st.sidebar.form("form_apuesta_partido"):
    ...
    seleccion_equipo = st.selectbox(
        "Equipo Seleccionado:", [away_name, home_name]
    )
  ...
  away_pitcher, home_pitcher = probables.get("away", {}), probables.get(
      "home", {}
  )
  ...
  prob_away, prob_home, sim_away, sim_home, total_esperado = (
      simular_monte_carlo(exp_runs_away, exp_runs_home, n_simulaciones)
  )
  ...
  with tab_montecarlo:
    ...
    c1.metric(
        f"Prob. Victoria {away_name}",
        f"{prob_away:.1f}%",
        f"Proj: {sim_away:.2f} R",
    )
    c2.metric(
        f"Prob. Victoria {home_name}",
        f"{prob_home:.1f}%",
        f"Proj: {sim_home:.2f} R",
    )
    c3.metric(
  ...
  with tab_ev:
    ...
    if not cuotas_raw:
      st.info(
          "💡 **Modo Demo**: Se muestran cuotas simuladas. Añade tu API Key para"
          " datos en vivo de Pinnacle, DraftKings, etc."
      )
      cuotas_lista = [
  ...
  with tab_vivo:
    ...
    if st.session_state.mis_apuestas:
      ...
      if apuestas_del_partido:
        ...
        st.info(
            "No tienes apuestas de partido registradas para este encuentro."
        )
    ...
    with col_campo:
      st.markdown(
          generar_campo_svg(offense, hit_x, hit_y), unsafe_allow_html=True
      )
    ...
    if entradas_lista:
      ...
      c_tot1.metric(
          f"Total {away_name}",
          f"R: {away_totals.get('runs', 0)} | H:"
          f" {away_totals.get('hits', 0)} | E:"
          f" {away_totals.get('errors', 0)}",
      )
      c_tot2.metric(
          f"Total {home_name}",
          f"R: {home_totals.get('runs', 0)} | H: {home_totals.get('hits', 0)}"
          f" | E: {home_totals.get('errors', 0)}",
      )
    else:
